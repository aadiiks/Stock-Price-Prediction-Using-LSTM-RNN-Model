...
data = web.DataReader('HINDPETRO.NS', 'yahoo', start, end)
data.head()
...
dataset_test = pd.read_csv('HINDPETROjan.NS.csv',index_col="Date",parse_dates=True)
...
#data = data.DataReader(name="TSLA", data_source='yahoo', start=start_date, end=end_date)
dataset_test = web.DataReader('HINDPETRO.NS', 'yahoo', start, end)
real_stock_price = dataset_test.iloc[:, 1:2].values
...
#Getting the Predicted stock Prices of Jan 2022
...
plt.plot(real_stock_price, color = 'red', label = 'Real HPCL Stock Price')
plt.plot(predicted_stock_price, color = 'blue', label = 'Predicted HPCL Stock Price')
plt.title('HPCL Stock Price Prediction')
plt.xlabel('Time')
plt.ylabel('HPCL Stock Price')
plt.legend()
plt.show()
...
#Finished
predicted_stock_price.to_csv('Stock_Price_Prediction_HPCL.csv')
